...
BEGIN
 ...
 BEGIN
  IF EXISTS(SELECT * FROM RoomsStudents rs 
			...
			WHERE s.StudentId=@StudentId) SET @LineAnswer='Данный студент уже заселен в комнату.'
 ...
 BEGIN
  IF EXISTS(SELECT * FROM RoomsStudents rs 
			WHERE rs.StudentId=@StudentId AND rs.RoomId=@RoomId) SET @LineAnswer='Студент уже заселен в выбранную Вами комнату.'
  IF EXISTS(SELECT * FROM Rooms r
			INNER JOIN TypeRooms tr ON r.TypeRoomId=tr.TypeRoomId
			WHERE r.RoomId=@RoomId AND tr.TypeRoomId=1) AND EXISTS(SELECT * 
																   FROM Students s 
